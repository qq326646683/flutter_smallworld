...
import 'package:flutter_smallworld/page/common/demo/drag_swipe_demo.dart';

// Examples can assume:
// class MyDataObject { }

/// The callback used by [SMReorderableListView] to move an item to a new
/// position in a list.
///
...
/// [SMReorderableListView] will need to account for this when inserting before
/// [newIndex].
///
/// {@tool sample}
///
...
class SMReorderableListView extends StatefulWidget {

  /// Creates a reorderable list.
  SMReorderableListView({
    this.header,
  ...
  /// underlying list.
  ///
  /// This [SMReorderableListView] calls [onReorder] after a list child is dropped
  /// into a new position.
  final ReorderCallback onReorder;

  @override
  _SMReorderableListViewState createState() => _SMReorderableListViewState();
}
...
class _SMReorderableListViewState extends State<SMReorderableListView> {
  // We use an inner overlay so that the dragging list item doesn't draw outside of the list itself.
  final GlobalKey _overlayKey = GlobalKey(
      debugLabel: '$SMReorderableListView overlay key');
...
class _ReorderableListContentState extends State<_ReorderableListContent>
  ...
  void _scrollTo(BuildContext context) {
    ...
    final bool onScreen = scrollOffset <= topOffset &&
    ...
    return LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          for (int i = 0; i < widget.children.length; i += 1) {
            if ((widget.children[i] as dynamic).itemData.dataType ==
                DataType.task) {
              wrappedChildren.add(_wrap(widget.children[i], i, constraints));
            } else if ((widget.children[i] as dynamic).itemData.dataType ==
                DataType.addBtn) {
